...
const p2pShareElement = document.querySelector('.wp-block-planet4-gpch-plugin-blocks-p2p-share');

const p2pStepElements = p2pShareElement.querySelectorAll(':scope .p2p-share-step');
...
nextButtons.forEach((item) => {
	item.addEventListener('click', (event) => {
		...
		const fieldToValidate = event.target.getAttribute('data-next-step-field');

		try {
			const selectedField = document.querySelector('input[name="' + fieldToValidate + '"]:checked');

			const nextElementSelector = selectedField.getAttribute('data-next-element');
...
// Back button events
const backButtons = p2pShareElement.querySelectorAll(':scope .controls .previous');

backButtons.forEach((item) => {
	item.addEventListener('click', (event) => {
		event.preventDefault();

		const previousElementSelector = event.target.getAttribute('data-previous-element');

		const prevStep = p2pShareElement.querySelectorAll(':scope ' + previousElementSelector);
...
const clipboardButtons = p2pShareElement.querySelectorAll(':scope .controls .copy-to-clipboard');
...
// Send by SMS
const smsButtons = p2pShareElement.querySelectorAll(':scope .controls .send-sms');
...
smsButtons.forEach((item) => {
	item.addEventListener('click', (event) => {
		...
		const defaultStateElements = sendButton.querySelectorAll(':scope .state-default');
		const progressStateElements = sendButton.querySelectorAll(':scope .state-progress');
		// Kept here for better readability
		// eslint-disable-next-line @wordpress/no-unused-vars-before-return
		const doneStateElements = sendButton.querySelectorAll(':scope .state-done');
		...
		const numberField = document.getElementById(sendButton.dataset.mobileNumberField);
		const statusElement = sendButton.closest('.option').querySelector(':scope .status');
		...
		try {
			...
			statusElement.innerText = __('Please use a valid Swiss mobile number.', 'planet4-gpch-plugin-blocks');
...
const smsInputFields = p2pShareElement.querySelectorAll(':scope .phone-number-field');
...
const emailInputFields = p2pShareElement.querySelectorAll(':scope .email-field');
...
// Send by email
const emailButtons = p2pShareElement.querySelectorAll(':scope .controls .send-email');
...
emailButtons.forEach((item) => {
	item.addEventListener('click', (event) => {
		...
		const channel = sendButton.dataset.channel;
		const emailField = document.getElementById(sendButton.dataset.emailField);
		const statusElement = sendButton.closest('.option').querySelector(':scope .status');
